    ...
    <div className="flex flex-col gap-6 w-full animate-fade-in font-sans">
      ...
      {filteredAssets.length === 0 ? (
        <div className="p-16 rounded-2xl border border-border border-dashed bg-card/30 text-center flex flex-col items-center gap-4">
          <div className="h-12 w-12 rounded-full bg-muted flex items-center justify-center text-zinc-500">
          ...
          {filteredAssets.map((asset) => {
            ...
            return (
              <div 
                ...
                className="group relative rounded-xl border border-border bg-card hover:border-zinc-350 dark:hover:border-zinc-850 hover:bg-zinc-50 dark:hover:bg-zinc-900/20 overflow-hidden cursor-pointer transition-all duration-300 flex flex-col h-48 shadow-lg"
              >
                {/* Media Preview Box */}
                <div className="flex-1 w-full bg-muted flex items-center justify-center relative overflow-hidden group-hover:scale-[1.02] transition-transform duration-500">
                ...
                {/* Footer details */}
                <div className="p-3 flex flex-col gap-1 border-t border-border bg-muted/40">
                  <span className="text-xs font-medium text-foreground truncate group-hover:text-indigo-650 dark:group-hover:text-indigo-400 transition-colors">
                    {asset.fileName}
                  </span>
                  <div className="flex items-center justify-between text-[9px] text-zinc-500 font-semibold uppercase">
                    ...
                    <span className="text-zinc-650 dark:text-zinc-600 capitalize">{asset.type}</span>
                  </div>
                </div>
              </div>
            );
          ...
          <div className="w-full max-w-md bg-background border border-border rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleUploadSubmit} className="p-6 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5 text-left">
                <label className="text-xs font-semibold text-zinc-555 dark:text-zinc-400">Select File</label>
                <div className="border border-dashed border-border bg-muted/40 rounded-xl p-6 text-center flex flex-col items-center gap-3">
                  ...
                  <label 
                    ...
                    className="px-3 py-1.5 bg-muted hover:bg-muted/80 text-foreground border border-border rounded-lg text-xs font-semibold cursor-pointer transition-colors"
                  >
                    Browse Files
                  </label>
                </div>
              ...
              <div className="flex flex-col gap-1.5 text-left">
                <label className="text-xs font-semibold text-zinc-555 dark:text-zinc-400">Target Campaign/Project *</label>
                <select 
              ...
              <div className="flex flex-col gap-1.5 text-left">
                <label className="text-xs font-semibold text-zinc-555 dark:text-zinc-400">Attach to Content Post (Optional)</label>
                <select 
                  value={uploadPostId} 
      ...
      {activeModal === "inspect" && selectedAsset && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-3xl bg-background border border-border rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
            
            {/* Asset Preview Frame (Left/Top) */}
            <div className="flex-1 bg-muted flex items-center justify-center p-6 relative min-h-[300px] md:min-h-[400px] border-b md:border-b-0 md:border-r border-border">
              {selectedAsset.type === "image" && selectedAsset.url ? (
                ...
                <div className="flex flex-col items-center gap-4 text-zinc-500 text-center">
                  ...
                  <div className="flex flex-col gap-1">
                    <span className="font-semibold text-foreground text-sm">{selectedAsset.fileName}</span>
                    <span className="text-[10px] text-zinc-550 font-bold uppercase">{selectedAsset.type}</span>
                  </div>
                  {selectedAsset.url && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer"
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-muted border border-border rounded-lg text-xs font-semibold text-indigo-650 dark:text-indigo-400 hover:text-indigo-500"
                    >
            ...
            <div className="w-full md:w-80 flex flex-col p-6 overflow-y-auto bg-background">
              
              {/* Heading details */}
              <div className="flex justify-between items-start gap-4 pb-4 border-b border-border mb-4">
                <div className="flex flex-col min-w-0">
                  <h4 className="font-bold text-foreground text-base truncate" title={selectedAsset.fileName}>
                    {selectedAsset.fileName}
                  </h4>
                  <span className="text-[10px] text-zinc-550 dark:text-zinc-500 uppercase font-bold mt-0.5 tracking-wider">
              ...
              {/* General Metadata */}
              <div className="flex flex-col gap-3 text-xs mb-6 bg-muted/40 p-3 rounded-lg border border-border">
              ...
              <form onSubmit={handleAttachSave} className="flex flex-col gap-3 flex-1">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-zinc-555 dark:text-zinc-400">Attached Content Post</label>
                  <select 
                    value={inspectPostId} 
                    onChange={(e) => setInspectPostId(e.target.value)}
                    className="w-full px-3 py-2 rounded-lg border border-border bg-muted text-foreground text-xs focus:outline-none focus:border-indigo-600"
                  >
                ...
                <div className="flex gap-2">
                  <Button 
                    ...
                    size="sm"
                    className="flex-1 bg-muted hover:bg-muted/80 text-indigo-650 dark:text-indigo-400 border border-border text-[11px]"
                  >
              ...
              <div className="border-t border-border pt-4 mt-6 flex flex-col gap-2">
                ...
                {selectedAsset.url && (
                  ...
                  >
                    <Button 
                      variant="outline" 
                      size="sm"
                      className="w-full border-border hover:bg-muted text-foreground text-xs"
                    >
                ...
                <Button 
                  ...
                  variant="ghost" 
                  size="sm"
                  className="w-full hover:bg-red-500/10 text-red-550 hover:text-red-500 text-xs"
                >
